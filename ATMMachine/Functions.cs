       ...
       internal static void DepositMoney(Account userAccount)
        {
            ...
            {
                Console.WriteLine("\nError: only numeric values are allowed, please try again.\n");
                Console.WriteLine("\nTo go back to main menu press any key");
                Console.ReadLine();
                Console.Clear();
            }           
        ...
        internal static void TransferMoney(Account userAccount, ListOfAllAccounts accountList)
        {
            try
            {
                ...
                if (userYesOrNo.Equals("y"))
                {
                    ...
                    if (AccToSendMoneyTo != null && AccToSendMoneyTo.AccountNumber != userAccount.AccountNumber)
                    {
                        ...
                        {
                            Console.WriteLine("\nNot possible! due to one of the following reason(s): " +
                                              "\n\nTransfer amount cannot exceed account balance" +
                                              "\nTransfer amount cannot be more than £500 \n");
                            Console.WriteLine("\nTo go back to main menu press any key");
                            Console.ReadLine();
                            Console.Clear();
                        }
                    ...
                    else if(AccToSendMoneyTo != null && userAccount.AccountNumber == AccToSendMoneyTo.AccountNumber)
                    {
                        Console.WriteLine("\nError: Cannot transfer money to own amount");
                        Console.WriteLine("\nTo go back to main menu press any key");
                        Console.ReadLine();
                        Console.Clear();
                    }
                ...
                else if (!userYesOrNo.Equals("y") || !userYesOrNo.Equals("n"))
                {
                    Console.WriteLine("\nProcess terminated due to invalid sellection\n");
                    Console.WriteLine("\nTo go back to main menu press any key");
                    Console.ReadLine();
                    Console.Clear();
                }
